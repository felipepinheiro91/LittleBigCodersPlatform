...
export function Metrics({ items }) {
  return <SimpleGrid columns={{ base: 1, sm: 2, xl: 4 }} gap="4">{items.map(([label, value, detail]) => <Box className="bi-kpi" key={label}><Text color="gray.600" fontSize="sm">{label}</Text><Heading size="2xl" my="2">{value ?? "—"}</Heading>{detail && <Text fontSize="sm" color="gray.500">{detail}</Text>}</Box>)}</SimpleGrid>;
}
export function ReportMetrics({ data }) {
  return <Metrics items={[["Estudantes", data.students, `${data.active_students} com prova concluída`], ["Acertos", percent(data.score), "Todas as tentativas concluídas"], ["Conclusão", percent(data.completion), `${data.completed} de ${data.proposed} provas atribuídas`], ["Tentativas concluídas", data.attempts]]} />;
}
